...
router.get("/classes", async (req, res, next) => {
  try {
    ...
    type TeacherCourse = (typeof classes)[number];
    const grouped: Record<string, { id: string; name: string; color: string; courses: TeacherCourse[] }> = {};
    for (const course of classes) {
      ...
      grouped[course.classId]!.courses.push(course);
    }
...
router.get("/classes/:id/sessions", async (req, res, next) => {
  try {
    const { id } = req.params;
